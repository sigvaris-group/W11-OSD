...
Write-Host -ForegroundColor Green "Installed languages: $($InstalledLanguages)"

    try {
        # Attempt to install the language
        Write-Host "    Installing language $($OSDLanguage)"
        Install-Language -Language $OSDLanguage -CopyToSettings          
    catch {
            Write-Host -ForegroundColor Red "Error installing language $($OSDLanguage). Error:  Error: $($_.Exception.Message). Exiting script"
